    ...
    <div class="container p-5">
        ...
        <form method="post" action="<?= BASE_URL ?>/index.php?action=<?= isset($utilisateur) ? 'editutilisateur&id=' . $utilisateur['id'] : 'createutilisateur' ?>">
            <div class="form-group">
                ...
                <input type="text" class="form-control" id="nomUtilisateur" name="username" value="<?= isset($utilisateur) ? $utilisateur['username'] : '' ?>" required>
            </div>
            <div class="form-group">
                <label for="role">Role</label>
                <select class="form-control" id="role" name="role" required>
                    <option value="admininistrateur" <?= isset($utilisateur) && $utilisateur['role'] == 'administrateur' ? 'selected' : '' ?>>Admin</option>
